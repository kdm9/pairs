...
usage()
{
    ...
    fprintf(stderr,
            "          split      get the nucleotide composition of FASTA/Q\n");
    return 1;
}
...
void printseq(FILE *stream, const kseq_t *s, int line_len, int tag)
{
    ...
    if (tag) {
        fputc('/', stream);
        putc((char)(((int)'0')+tag), stream);
    }

    if (s->comment.l) {
        fputc(' ', stream);
        fputs(s->comment.s, stream);
    }
...
int pairs_split(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "1:2:u:n")) >= 0) {
        ...
        case 'm':
            ...
            if (min_length < 1) {
                fprintf(stderr, "[%s] error: minimum length must be >= 1\n", __func__);
            }
            return 1;
            break;
        case 'n':
    ...
    fp = (strcmp(argv[optind], "-") == 0) ? gzdopen(fileno(stdin),
            "r") : gzopen(argv[optind], "r");

    seq = calloc(2, sizeof(kseq_t *));
    for (i = 0; i < 2; ++i) {
        seq[i] = malloc(sizeof(kseq_t));
    }
    tmp = kseq_init(fp);
    ...
    if (total[0] != total[1]) {
        fprintf(stderr,
                "[%s] error: mismatched totals of interleaved pairs! %u != %u\n", __func__,
                total[0], total[1]);
        return 1;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(argv[1], "join") == 0) {
        return pairs_join(argc-1, argv+1);
    }
    if (strcmp(argv[1], "split") == 0) {
        return pairs_split(argc-1, argv+1);
    } else {
